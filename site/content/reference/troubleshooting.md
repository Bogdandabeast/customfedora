...
| `ryzen-profiles` `max 2000000` after `power-saver` or after `resume` | `PPD power-saver` clamps `scaling_max` to `2.0GHz`; `SMU` también se pierde al `resume` | `ryzen-resume.service` re-aplica `ADP1` + `EC 80%` tras `suspend-then-hibernate`/`hybrid-sleep`; manual: `just ryzen-perf` / `just ryzen-silent-gaming on` / `just ryzen-battery` — they set `2900000`/`2200000`/`4547946` |
