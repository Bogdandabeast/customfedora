...
if [ "$MODE" = "on" ]; then
  ...
  if [ "$ADP1" != "1" ]; then
    ...
    /usr/bin/ryzen-profiles battery 2>&1 | tail -n 10 || true
    exit 1
  fi
  mkdir -p /var/lib
  date -Iseconds > "$FLAG"
  echo "Flag silent-gaming creado: $FLAG (solo enchufado)"
  /usr/bin/ryzen-profiles silent-gaming
  echo ""
  ...
  if [ "$ADP1" = "1" ]; then
    ...
    /usr/bin/ryzen-perf-hook.sh 2>&1 || /usr/bin/ryzen-profiles perf
  else
    echo "ADP1=0 -> volviendo a ULTRA 6W"
    /usr/bin/ryzen-battery-hook.sh 2>&1 || /usr/bin/ryzen-profiles battery
  fi
